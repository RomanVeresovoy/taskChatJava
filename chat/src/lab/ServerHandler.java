        ...
        try {
            ...
            while (true) {
                ...
                if (message.getMessage().equals("/leave")) {
                    Server.clients.leaveClient(message.getId());
                    continue;
                }
                ...
                String complite = message.getName() + " (" + dateFormat.format(message.gerDate())
                        + ") " + ": " + message.getMessage();
                if (message.getId() == 0) {
                    if (Server.clients.firstSendMess(reverseName, complite, message.getI()))
                        this.sendMsg("*" + Clients.kk + "*");
                    else
                        this.sendMsg("null");
                } else
                    Server.clients.sendMess(message.getId(), reverseName, complite);
            }
        } catch (IOException e) {
            System.err.println("Error I/OOO");
